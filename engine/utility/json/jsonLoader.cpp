...
void JsonLoader::LoadSceneFile(const std::string &filePath) {
    ...
    for (nlohmann::json &object : deserialized["objects"]) {
        ...
        if (type.compare("MESH") == 0) {
            ...
            PlayerData &playerData = levelData_->players.back();
            ...
            // 回転角の数値をラジアンできき込む
            const float degToRad = (float)std::numbers::pi / 180.0f;
            playerData.rotation.x = -(float)transform["rotation"][0] * degToRad;
            playerData.rotation.y = -(float)transform["rotation"][2] * degToRad;
            playerData.rotation.z = -(float)transform["rotation"][1] * degToRad;
        }

        else if (type.compare("EnemySpawn") == 0) {
            // enemiesに要素を1つ追加する
            levelData_->enemies.push_back(EnemyData{});
            EnemyData &enemyData = levelData_->enemies.back();
            ...
            // 回転角の数値をラジアンできき込む
            const float degToRad = (float)std::numbers::pi / 180.0f;
            enemyData.rotation.x = -(float)transform["rotation"][0] * degToRad;
            enemyData.rotation.y = -(float)transform["rotation"][2] * degToRad;
            enemyData.rotation.z = -(float)transform["rotation"][1] * degToRad;
        }
    }
...
const std::vector<JsonLoader::PlayerData> &JsonLoader::GetPlayers() const {
    if (levelData_) {
        return levelData_->players;
    }
    static const std::vector<JsonLoader::PlayerData> empty;
    return empty;
}

const std::vector<JsonLoader::EnemyData> &JsonLoader::GetEnemies() const {
    if (levelData_) {
        return levelData_->enemies;
    }
    static const std::vector<JsonLoader::EnemyData> empty;
    return empty;
}
